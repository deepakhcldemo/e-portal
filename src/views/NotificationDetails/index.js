        ...
        return (
            ...
            <div className="container-fluid">
                ...
                <div className ="card col-lg-8" style = {{margin : 'auto'}}>
                    <div>
                        <span className="bold">Notification Description : {this.state.notificationsDetails.notificationDesc}</span>
                    </div>
                    <div> <span className="bold"> Video upload BY :</span> {this.state.userDetails.role === "Teacher" ? "Student" : "Teacher"}</div>
                    <ModalPopUpForNotification teacherNotofication={this.state.notificationsDetails} />
                    <video src={this.state.notificationsDetails.sVideo || this.state.notificationsDetails.tVideo}></video>
                    {this.state.userDetails.role === "Teacher" ? <div className ="col-lg-12">
                    <div className ="pull-left">
                    <button className="btn btn-dark" onClick={() => this.props.history.goBack()} >Back</button>
                    </div>
                    <div className ="pull-right">
                        
                        <button className="btn btn-success" onClick={this.acceptNotification} >Accept
                        </button>
                        <button className="btn btn-danger" onClick={this.rejectNotification}>Reject</button>
                    </div></div> : null}
                </div>
                </div>
            </div>)
...
const mapDispatchToProps = dispatch => {
    return {
        ...
        openModalForAcceptNotification: () => dispatch(openModalForAcceptNotification())
    };
};
